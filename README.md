...
[![pre-commit](https://github.com/lablabs/terraform-aws-eks-github-actions-runner/actions/workflows/pre-commit.yml/badge.svg)](https://github.com/lablabs/terraform-aws-eks-github-actions-runner/actions/workflows/pre-commit.yml)

## Description

A Terraform module to deploy the github-actions-runner on Amazon EKS cluster. This addon does not install the controller itself, just instances of runners. Use [github-actions-runner-controller](https://github.com/lablabs/terraform-aws-eks-github-actions-runner-controller) to install the controller beforehand.
...
| <a name="input_helm_release_name"></a> [helm\_release\_name](#input\_helm\_release\_name) | Helm release name | `string` | `"github-actions-runner"` | no |
...
| <a name="input_helm_repo_url"></a> [helm\_repo\_url](#input\_helm\_repo\_url) | Helm repository | `string` | `"https://lablabs.github.io/github-actions-runners-helm/"` | no |
...
| <a name="input_irsa_role_name_prefix"></a> [irsa\_role\_name\_prefix](#input\_irsa\_role\_name\_prefix) | The IRSA role name prefix for vector | `string` | `"github-actions-runner-irsa"` | no |
| <a name="input_irsa_tags"></a> [irsa\_tags](#input\_irsa\_tags) | IRSA resources tags | `map(string)` | `{}` | no |
| <a name="input_namespace"></a> [namespace](#input\_namespace) | The K8s namespace in which the github-actions-runner service account has been created | `string` | `"github-actions-runner"` | no |
...
| <a name="input_service_account_name"></a> [service\_account\_name](#input\_service\_account\_name) | The k8s github-actions-runner service account name | `string` | `"github-actions-runner"` | no |
| <a name="input_settings"></a> [settings](#input\_settings) | Additional helm sets which will be passed to the Helm chart values, see https://hub.helm.sh/charts/stable/github-actions-runner | `map(any)` | `{}` | no |
| <a name="input_values"></a> [values](#input\_values) | Additional yaml encoded values which will be passed to the Helm chart, see https://hub.helm.sh/charts/stable/github-actions-runner | `string` | `""` | no |
